...
import { abi as XShareAbi } from "./abi/XShare.json";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const farmingIncentiveFund = GraveDeployConfig.farmingIncentiveFund;

  const XShare = await ethers.getContractAt(XShareAbi, XShareAddress);
  if ((await XShare.rewardPoolDistributed()) == false) {
    console.log("XShare reward pool distributing... ");
    await (
      await XShare.distributeReward(farmingIncentiveFund, {
        gasLimit: gasLimit,
      })
    ).wait();
    console.log("XShare reward pool distributing... ok");
  } else {
    console.log("XShare reward pool already ");
  }
};

func.tags = ["XShareDistributeRewards"];
