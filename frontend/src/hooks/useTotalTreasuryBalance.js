...
function useTotalTreasuryBalance() {
    ...
    const COUSD = new web3.eth.Contract(ERC20ABI, '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83')
    const [balance, setBalance] = useState(0)
    const [balance_2shares_wftm, setBalance_2shares_wftm] = useState(0)
    const [balance_xGRAVE_wftm, setBalance_xGRAVE_wftm] = useState(0)
    const [balance_3shares_wftm, setBalance_3shares_wftm] = useState(0)
    const [balance_xGRAVE, setBalance_xGRAVE] = useState(0)
    ...
    return { balance, balance_2shares_wftm, balance_xGRAVE_wftm, balance_3shares_wftm, balance_xGRAVE, balance_3shares, balance_2shares }
    ...
    async function getBalance() {
        ...
        // const { dataxGRAVE } = await axios('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=30mb-token')
        ...
        const data2sharesAndxGRAVE = await axios('https://openapi.debank.com/v1/user/chain_balance?id=0x8f555E00ea0FAc871b3Aa70C015915dB094E7f88&chain_id=ftm')

        console.log(`xShares USD: $${value3shares}`)
        console.log(`2Shares + xGRAVE: $${data2sharesAndxGRAVE.data.usd_value}`)
        ...
        const LP_xGRAVE_wftm = await getLPPrice('0x83A52eff2E9D112E9B022399A9fD22a9DB7d33Ae', '0x14def7584a6c52f470ca4f4b9671056b22f4ffde')
        ...
        setBalance(data2sharesAndxGRAVE.data.usd_value + value3shares + LP_2shares_wftm + LP_xGRAVE_wftm + LP_3shares_wftm)
        setBalance_2shares_wftm(LP_2shares_wftm)
        setBalance_xGRAVE_wftm(LP_xGRAVE_wftm)
        setBalance_3shares_wftm(LP_3shares_wftm)
        setBalance_xGRAVE(await getxGRAVEBalance())
        setBalance_3shares(await get3sharesBalance())
        setBalance_2shares(await get2sharesBalance())
    }

    async function getxGRAVEBalance() {
        const tokenxGRAVE = new web3.eth.Contract(ERC20ABI, '0x14DEf7584A6c52f470Ca4F4b9671056b22f4FfDE')
        ...
        const usdValue = Number(web3.utils.fromWei(await tokenxGRAVE.methods.balanceOf(treasuryAddress).call())) * Number(data.price)
    ...
    async function getLPPrice(LPAddress, tokenAddress) {
        ...
        const wftmValue = Number(web3.utils.fromWei(await COUSD.methods.balanceOf(LPAddress).call())) * Number(data.price)
