...
export class GraveyardFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.XGRAVE = new ERC20(deployments.grave.address, provider, 'GRAVE');
  ...
  async getXgraveStat(): Promise<TokenStat> {
    ...
    const graveRewardPoolSupply = await this.XGRAVE.balanceOf(XgraveFtmRewardPool.address);
    const graveRewardPoolSupply2 = await this.XGRAVE.balanceOf(XgraveFtmLpXgraveRewardPool.address);
    const graveRewardPoolSupplyOld = await this.XGRAVE.balanceOf(XgraveFtmLpXgraveRewardPoolOld.address);
    ...
    return {
      ...
      circulatingSupply: getDisplayBalance(graveCirculatingSupply, this.XGRAVE.decimal, 0),
    };
  }
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const graveStat = await this.getXgraveStat();
    ...
    const bondPriceInFTM = (Number(graveStat.tokenInFtm) * modifier).toFixed(2);
  ...
  async gexShareStat(): Promise<TokenStat> {
    ...
    const graveRewardPoolSupply = await this.XSHARE.balanceOf(XgraveFtmLPXShareRewardPool.address);
   ...
   * @param isXgrave sanity check for usage of grave token or xShare
...
async get2ombStatFake(): Promise<TokenStat> {
  ...
  const graveRewardPoolSupply = await XGRAVE.balanceOf(TwoOmbFtmRewardPool.address);
  const graveRewardPoolSupply2 = await XGRAVE.balanceOf(TwoOmbFtmLpXgraveRewardPool.address);
  const graveRewardPoolSupplyOld = await XGRAVE.balanceOf(TwoOmbFtmLpXgraveRewardPoolOld.address);
  const graveCirculatingSupply = supply
    .sub(graveRewardPoolSupply)
    .sub(graveRewardPoolSupply2)
    .sub(graveRewardPoolSupplyOld);
  ...
  return {
    ...
    circulatingSupply: getDisplayBalance(graveCirculatingSupply, XGRAVE.decimal, 0),
  };
}
...
async get2ShareStatFake(): Promise<TokenStat> {
  ...
  const graveRewardPoolSupply2 = await XSHARE.balanceOf(TwoOmbFtmLpXgraveRewardPool.address);
  const graveRewardPoolSupplyOld = await XSHARE.balanceOf(TwoOmbFtmLpXgraveRewardPoolOld.address);
  const graveCirculatingSupply = supply
    .sub(graveRewardPoolSupply)
    .sub(graveRewardPoolSupply2)
    .sub(graveRewardPoolSupplyOld);
  ...
  async provideXgraveFtmLP(usdcAmount: string, graveAmount: BigNumber): Promise<TransactionResponse> {
  ...
  async getXShareSwapperStat(address: string): Promise<XShareSwapperStat> {
    ...
    // const gravePriceBN = await XShareSwapper.getXgravePrice();
